...
def _edge_candidates() -> list[str]:
    ...
    potential_roots = [program_files, program_files_x86, local_app_data]
...
def launch_browser_in_edge(url: str) -> None:
    """Try to open the dashboard in Microsoft Edge on Windows."""
    ...
    def _launch() -> None:
        ...
        for candidate in _edge_candidates():
            if os.path.exists(candidate):
                webbrowser.register('msedge', None, webbrowser.BackgroundBrowser(candidate))
                try:
        ...
        if not browser_opened:
            try:
                ...
                print('Unable to launch a browser automatically. Open the dashboard manually.')
...
def main() -> None:
    ...
    print(f"Serving dashboard on {url} (snapshot: {args.snapshot})")

    launch_browser_in_edge(url)
    try:
